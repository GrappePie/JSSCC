...
with sync_playwright() as p:
 ...
 try:
  ...
  check('new product identity is visible',page.title()=='JSSCC — PCM Edition · GrappePie' and 'v0.1.0' in page.locator('#jsscc-edition').inner_text())
  check('canvas notices preserve upstream copyright and new version',page.evaluate("JSON.stringify(ui.renderer.loader.drawGroups).includes('(C) 2017 meme.institute + Milkey Mouse') && JSON.stringify(ui.renderer.loader.drawGroups).includes('PCM v0.1.0')"))
  ...
  with page.expect_file_chooser() as fc:card.locator('.edition-play').click()
  ...
  check('associated card now offers actual one-click local playback','Reproducir chiptune' in page.locator('.edition-card').first.locator('.edition-play').inner_text())
  page.locator('.edition-card').first.locator('.edition-play').click();page.wait_for_function("JSSCCMidi.diagnostics().state==='playing'")
