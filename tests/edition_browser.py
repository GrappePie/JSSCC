...
import functools,http.server,threading,sys,shutil,json,re
...
with sync_playwright() as p:
 ...
 try:
  ...
  check('no Online Sequencer requests before opening its drawer',len(external)==0,external)
  ...
  check('external thumbnails are enabled by default',page.locator('#edition-thumbnails').is_checked())
  page.wait_for_timeout(250)
  thumb_requests=[u for u in external if re.match(r'^https://onlinesequencer\.net/t/\d+/[\w.-]+\.(?:gif|png|jpg|jpeg|webp)$',u,re.I)]
  check('opening the drawer may load only official static thumbnails by default',len(thumb_requests)>0 and len(thumb_requests)==len(external),external)
  ...
  page.set_input_files('#jsscc-file',{'name':'different.mid','mimeType':'audio/midi','buffer':midi});page.wait_for_function("JSSCCMidi.diagnostics().state==='playing'")
  check('main file picker autoplays newly loaded MIDI',page.evaluate("JSSCCMidi.current?.fileName==='different.mid'"))
  page.wait_for_function("document.querySelector('#jsscc-sequence-source').hidden")
  check('replacing MIDI elsewhere clears stale source credit',True)
  check('loop is enabled by default',page.evaluate('ui.song.repeat===true'))
  ...
  check('thumbnail traffic stays constrained to official static assets',all(re.match(r'^https://onlinesequencer\.net/t/\d+/[\w.-]+\.(?:gif|png|jpg|jpeg|webp)$',u,re.I) for u in external),external)
